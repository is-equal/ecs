...
function debugQuadTree(quadtree: ReadonlyQuadTree | undefined): void {
  ...
  if (!quadtree.isSubdivided()) {
    return debugQuadTreeNodes(quadtree.getNodes());
  }
...
function debugQuadTreeNodes(nodes: ReadonlyArray<Readonly<Rect>>): void {
  transaction(() => {
